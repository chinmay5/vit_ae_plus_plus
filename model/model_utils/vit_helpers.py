...
def get_1d_sincos_pos_embed_from_grid(embed_dim, pos):
    ...
    """
    assert embed_dim % 2 == 0
    omega = np.arange(embed_dim // 2, dtype=np.float)  # Default step size is 1 [1,2,3...]
...
def interpolate_pos_embed(model, checkpoint_model):
    if 'pos_embed' in checkpoint_model:
        ...
        # Find if we have only CLS token or both CLS and DEIT token. These are the extra tokens.
        num_extra_tokens = model.pos_embed.shape[-2] - num_patches
        # height (== width) for the checkpoint position embedding
        orig_size = int((pos_embed_checkpoint.shape[-2] - num_extra_tokens) ** (1 / 3))
        # height (== width) for the new position embedding
        new_size = int(num_patches ** (1 / 3))
        # class_token and dist_token are kept unchanged
        if orig_size != new_size:
            print("Position interpolate from %dx%dx%d to %dx%dx%d" % (
            orig_size, orig_size, orig_size, new_size, new_size, new_size))
            # remove the cls/deit tokens from the interpolation process
            ...
            pos_tokens = pos_tokens.permute(0, 2, 3, 4, 1).flatten(start_dim=1, end_dim=3)
