...
def get_dataset(dataset_name, mode, args, transforms=None, use_z_score=False):
    assert dataset_name in ['large_brats', 'brats', 'breast_cancer'], f"Unsupported dataset {dataset_name}"
    if dataset_name == 'large_brats':
        return large_brats_data.create_the_dataset(mode=mode, args=args, transforms=transforms, use_z_score=use_z_score)
