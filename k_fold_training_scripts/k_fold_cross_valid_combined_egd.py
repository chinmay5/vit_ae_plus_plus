...
def main(args):
    ...
    dataset_labeled = get_dataset(dataset_name='egd', mode='test', args=args, transforms=train_transforms,
                                use_z_score=args.use_z_score)
    dataset_labeled_no_aug = get_dataset(dataset_name='egd', mode='test', args=args, transforms=None,
                                   use_z_score=args.use_z_score)
    # Since we do not want any augmentation on the test set.
    dataset_ssl = get_dataset(dataset_name='egd', mode='ssl', args=args, transforms=train_transforms,
                                       use_z_score=args.use_z_score)
    # We would split the training dataset for stratified k fold
    # Hard-coding the in channels
    args.in_channels = 4
    features, labels = get_all_feat_and_labels(dataset_labeled_no_aug, args=args)
    ...
    for idx, (train_ids, test_ids) in enumerate(kfold_splits.split(features, labels)):
        ...
        # Concatenate the training dataset with the SSL dataset
        combined_dataset = torch.utils.data.ConcatDataset([dataset_labeled, dataset_ssl])
        # Now, also update the indices to include the ssl dataset
        ssl_ids = np.asarray([x + len(dataset_labeled) for x in range(len(dataset_ssl))])
        whole_train_ids = np.concatenate([train_ids, ssl_ids])
        # Needed for the pre-training phase
        args.nb_classes = 2
        #
        # # Now we create the dataloader
        train_subsampler = torch.utils.data.SubsetRandomSampler(whole_train_ids)
        test_subsampler = torch.utils.data.SubsetRandomSampler(test_ids)
        #
        data_loader_train = torch.utils.data.DataLoader(
            combined_dataset, sampler=train_subsampler,
        ...
        data_loader_test = torch.utils.data.DataLoader(
            dataset_labeled_no_aug, sampler=test_subsampler,
            batch_size=args.batch_size,
            num_workers=args.num_workers,
            pin_memory=args.pin_mem,
            drop_last=False
        ...
        ssl_feature_dir = os.path.join(PROJECT_ROOT_DIR, 'egd', 'ssl_features_dir', args.subtype)
        ...
        # We would use the version with no augmentation in this case.
        dataset_no_aug_train = torch.utils.data.Subset(dataset_labeled_no_aug, train_ids)
        dataset_no_aug_test = torch.utils.data.Subset(dataset_labeled_no_aug, test_ids)
